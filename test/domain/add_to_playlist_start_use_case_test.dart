...
void main() {
  late AddToPlaylistStartUseCase useCase;
  MockPlaylistRepository mockRepository = MockPlaylistRepository();
  Invoker invoker = Invoker();

  setUpAll(() async {
    getTranslations();
    useCase = AddToPlaylistStartUseCase(repository: mockRepository);
  });

  test('that invoke calls addEpisodeAtStart on the repository', () {
    ...
    invoker.execute(useCase).listen(expectAsync1((result) {
      verify(mockRepository.addEpisodeAtStart(episode, 'Spoiler', 'assets/graphics/cuac-logo.png')).called(1);
